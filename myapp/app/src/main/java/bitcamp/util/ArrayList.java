...
//  // 3) non-static nested 클래스를 사용한 경우
//  @Override
//  public Iterator<E> iterator() {
...
//
//    바깥클래스의 인스턴스 주소를 두는 코드가 자동으로 추가된다.
//    int cursor;
//    public IteratorImpl(){
//    }
//    @Override
...
//    Iterator<E> obj = new Iterator<E>(){ // 여기서 "Iterator<E>()"는 익명클래스는 Iterator라는 슈퍼클래스의 생성자를 생성하는 것이다.
//                                         // 익명클래스는 무조건 상속받을때만 사용가능.
//
//      // 로컬 클래스도 non-static nested 클래스처럼
//      // 바깥 클래스의 인스턴스 주소를 저장하는 코드가 자동으로 추가된다.
//      int cursor;
...
//  }
  ...
  // 6) 익명 클래스인 경우 - 더 간결하게 표현하기
  @Override
  public Iterator<E> iterator() {
    // 익명 클래스는 이름이 없기 때문에 정의하는 즉시 인스턴스를 생성해야 한다.
    return new Iterator<E>(){ // 여기서 "Iterator<E>()"는 익명클래스는 Iterator라는 슈퍼클래스의 생성자를 생성하는 것이다.
      // 익명클래스는 무조건 상속받을때만 사용가능.
